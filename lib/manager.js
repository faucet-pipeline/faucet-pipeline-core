...
module.exports = class AssetManager {
	...
	writeFile(filepath, data, { error, target } = {}) {
		...
		return createFile(filepath, data).
			then(_ => this.manifest && this._updateManifest(originalPath, filepath, target)).
			then(_ => {
	...
	_updateManifest(originalPath, actualPath, target) {
		...
		return this.manifest.set(originalPath, actualPath, target);
	}
