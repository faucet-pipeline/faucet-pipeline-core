...
module.exports = class AssetManager {
	constructor(referenceDir, { manifestConfig, fingerprint, exitOnError } = {}) {
		...
		if(manifestConfig) {
			...
			this.manifest = new Manifest(manifestPath, manifestConfig.baseURI);
		}
	...
	writeFile(filepath, data, { error } = {}) {
		...
		return createFile(filepath, data).
			then(_ => this.manifest && this._updateManifest(originalPath, filepath)).
			then(_ => {
	...
	_updateManifest(originalPath, actualPath) {
		...
		return this.manifest.set(originalPath, actualPath);
	}
