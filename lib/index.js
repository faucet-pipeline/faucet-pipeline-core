...
exports.faucetDispatch = async function faucetDispatch(referenceDir, config,
		{ serve, liveserve, ...options }) {
	config = await config;
	let res = buildOnce(referenceDir, config, options);
...
exports.buildOnce = async function buildOnce(referenceDir, config,
		{ watch, fingerprint, sourcemaps, compact }) {
	config = await config;
